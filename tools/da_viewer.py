...
from litie.nn.uie import UIEModel
from uie_inference import inference
...
def get_prompt_samples(name: str) -> List[str]:
    ...
    with st.expander('', expanded=True):
        ...
        if upload_file:
            uploaded_files = st.file_uploader(f"选择文件", accept_multiple_files=True, key="a")
            for uploaded_file in uploaded_files:
                stringio = StringIO(uploaded_file.getvalue().decode("utf-8"))
                string_data = stringio.read()
                cur_data = string_data.split('\n')
                all_samples.extend([json.loads(line) for line in cur_data if line])
                files_count += 1
                df_dict = {'text': cur_data[:5] + ['...']}
                df = pd.DataFrame.from_dict(df_dict)
                st.dataframe(df, use_container_width=True)
        else:
            dataset_path_str = st.text_input(
                f'「训练数据集」的服务器路径（多个文件用英文逗号分隔）用于：{name}',
...
def get_doccano_samples():
    ...
    with st.expander('', expanded=True):
        ...
        if upload_file:
            uploaded_files = st.file_uploader("选择文件", accept_multiple_files=True, key="b")
            for uploaded_file in uploaded_files:
                stringio = StringIO(uploaded_file.getvalue().decode("utf-8"))
                string_data = stringio.read()
                cur_data = string_data.split('\n')
            ...
            dataset_path_str = st.text_input(
                f'「训练数据集」的服务器路径，jsonl格式（多个文件用英文逗号分隔）',
            ...
            if model_path and tokenizer_path:
                if not os.path.exists(model_path):
                    ...
                    with st.spinner('模型加载中，请稍等...'):
                        model = UIEModel.from_pretrained(model_path)
                        if device == 'cuda':
                            model.to(device)
                    model.eval()
                    st.success('🎉 模型加载成功~')
...
def main():
    st.title("Auto Data Augment For UIE")
    main_external_css = """
        <style>
